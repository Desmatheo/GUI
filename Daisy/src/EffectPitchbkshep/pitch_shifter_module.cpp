...
const uint32_t k_defaultSamplesDelayPitchShifter = 6000;
...
void PitchShifterModule::ProcessSemitoneTargetChange() {
    // Récupère la valeur mappée automatiquement entre -24.0 et 24.0
    float semitoneVal = GetParameterAsFloat(SEMITONE);

    // On arrondit à l'entier le plus proche pour avoir des "crans" précis (pas de microtonal)
    m_semitoneTarget = std::round(semitoneVal);
}
